...
namespace RHI::Vulkan
{
    ...
    GraphicsPipelineHandle Device::createGraphicsPipeline(const GraphicsPipelineDesc& desc, IFramebuffer* framebuffer)
    {
        ...
        std::vector<VkVertexInputAttributeDescription> attributeDescriptions;
        if (desc.inputLayout) {
            bindingDescriptions = getBindingDescription(desc.inputLayout.get());
            attributeDescriptions = getAttributeDescriptions(desc.inputLayout.get());
        ...
        rasterizer.polygonMode = convertFillMode(desc.renderState.fillMode);
        rasterizer.cullMode = convertCullMode(desc.renderState.cullMode);
